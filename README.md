...
1. Событие поступает из сырого источника.
2. Парсер извлекает базовые поля.
3. EventNormalizerAgent анализирует описание события.
4. Агент определяет формат, город, уровень, тематику и целевую аудиторию.
5. Нормализованное событие сохраняется в БД.
6. Рекомендательный модуль использует события для персонального подбора.
